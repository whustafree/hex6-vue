...
const fotos = ref([null, null, null]) // Array para manejar 3 fotos
...
<template>
  <div class="max-w-3xl mx-auto pb-20 animate-in fade-in duration-500">
    ...
    <div class="bg-slate-800 p-8 rounded-3xl border border-slate-700 shadow-2xl">
      <h2 class="text-3xl font-black italic text-purple-400 flex items-center gap-3 mb-8 uppercase">
      ...
      <form @submit.prevent="subirPublicacion" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="space-y-2">
            <label class="text-xs font-black uppercase text-slate-500">Nombre del Artículo</label>
            <input v-model="nuevoItem.item_nombre" type="text" required class="input-hex" placeholder="Ej: PlayStation 5 Slim">
          </div>
          <div class="space-y-2">
            <label class="text-xs font-black uppercase text-slate-500">Categoría</label>
            <select v-model="nuevoItem.categoria" class="input-hex appearance-none">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="space-y-2">
            ...
            <input v-model="nuevoItem.precio" type="number" required class="input-hex" placeholder="50000">
          </div>
          <div class="space-y-2">
            <label class="text-xs font-black uppercase text-slate-500">Tu WhatsApp (Sin +)</label>
            <input v-model="nuevoItem.telefono" type="text" required class="input-hex" placeholder="56912345678">
          </div>
        </div>

        <div class="space-y-2">
          <label class="text-xs font-black uppercase text-slate-500">Descripción / Estado</label>
          <textarea v-model="nuevoItem.descripcion" rows="3" class="input-hex" placeholder="Describe los detalles del producto..."></option>
        </div>

        <button :disabled="enviando" type="submit" class="w-full bg-purple-600 hover:bg-purple-500 text-white font-black py-4 rounded-2xl transition-all shadow-lg shadow-purple-900/40 uppercase tracking-widest flex items-center justify-center gap-2">
          <Loader2 v-if="enviando" class="w-5 h-5 animate-spin" />
          <CheckCircle v-else class="w-5 h-5" />
          {{ enviando ? 'Subiendo Galería...' : 'Publicar en Vitrina' }}
        </button>
      </form>
    </div>
  </div>
...
<style scoped>
.input-hex {
  @apply w-full bg-slate-900 border border-slate-700 text-white px-4 py-3 rounded-xl focus:outline-none focus:border-purple-500 transition-colors placeholder:text-slate-600 font-bold;
}
</style>