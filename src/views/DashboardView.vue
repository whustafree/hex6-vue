...
import { 
  ...
  MessageCircle, Send, CornerDownRight, BellRing
} from 'lucide-vue-next'

const cargando = ref(true)
const activeTab = ref('mensajes') // Cambiamos por defecto para que veas tus mensajes al entrar
const usuarioEmail = ref('')

const misCartas = ref([])
const misArticulos = ref([])
const misGrupos = ref([])
const misMensajes = ref([]) // NUEVA BANDEJA DE ENTRADA

const modalEdicion = ref(false); const itemEditando = ref(null); const tipoEditando = ref(''); const guardando = ref(false)
const respuestasPendientes = ref({}) // Para responder desde el dashboard
...
const cargarMisPublicaciones = async () => {
  try {
    ...
    // NUEVO: CARGAR BANDEJA DE ENTRADA (PREGUNTAS)
    ...
    if (msjs && msjs.length > 0) {
      // Buscar nombres de quienes te preguntaron
...
// Calcular preguntas sin responder para la campanita roja
const preguntasSinResponder = computed(() => misMensajes.value.filter(m => !m.respuesta).length)

// Función para responder directamente desde la bandeja
const enviarRespuesta = async (preguntaId) => {
  ...
  try {
    ...
    showToast('Respuesta enviada. El comprador la verá en el artículo.', 'success')
  } catch(e) { 
  ...
  <div class="space-y-8 pb-20 animate-in fade-in duration-500 max-w-5xl mx-auto p-4">
    ...
    <div v-else class="space-y-6">
      ...
      <div class="flex overflow-x-auto gap-2 p-2 bg-slate-800 border border-slate-700 rounded-2xl snap-x">
        <button @click="activeTab = 'mensajes'" :class="activeTab === 'mensajes' ? 'bg-orange-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-slate-700'" class="flex-1 flex items-center justify-center gap-2 py-3 px-6 rounded-xl font-black uppercase text-xs tracking-widest transition-all snap-center whitespace-nowrap relative">
          <MessageCircle class="w-4 h-4" /> Mensajes
          <span v-if="preguntasSinResponder > 0" class="absolute -top-1 -right-1 bg-red-500 text-white w-5 h-5 flex items-center justify-center rounded-full text-[10px] animate-pulse">{{ preguntasSinResponder }}</span>
        </button>
      ...
      <div v-show="activeTab === 'mensajes'" class="animate-in slide-in-from-left-4 duration-300">
        <div v-if="misMensajes.length === 0" class="text-center py-20 bg-slate-800/50 rounded-3xl border-2 border-dashed border-slate-700">
          <BellRing class="w-12 h-12 mx-auto text-slate-600 mb-4" />
          <p class="text-slate-500 font-bold uppercase tracking-widest">Tu bandeja está vacía</p>
        </div>
        <div v-else class="space-y-4">
          <div v-for="msj in misMensajes" :key="msj.id" class="bg-slate-800 p-5 rounded-2xl border transition-all" :class="msj.respuesta ? 'border-slate-700 opacity-75' : 'border-orange-500 shadow-lg shadow-orange-900/20'">
            <div class="flex justify-between items-start mb-3">
              <span class="text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded" :class="msj.tipo_item === 'tcg' ? 'bg-sky-500/20 text-sky-400' : 'bg-purple-500/20 text-purple-400'">En {{ msj.tipo_item }}</span>
              <span v-if="!msj.respuesta" class="text-[10px] bg-red-500 text-white px-2 py-1 rounded font-bold uppercase">Sin Responder</span>
            ...
            <div class="flex items-center gap-2 mb-2">
              ...
              <span class="text-xs font-black uppercase text-orange-400">{{ msj.remitente_nombre }} pregunta:</span>
            </div>
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div v-for="item in misCartas" :key="item.id" class="bg-slate-800 rounded-2xl p-4 border border-slate-700 flex flex-col justify-between">
            <div class="flex gap-4 items-start">
              ...
              <div><span class="text-[9px] bg-slate-900 px-2 py-1 rounded text-slate-400 uppercase font-bold">{{ item.juego }}</span><h4 class="font-bold text-white leading-tight mt-2">{{ item.titulo }}</h4><p class="text-sm text-green-400 font-black mt-1">${{ formatearPrecio(item.precio) }}</p></div>
            </div>
            <div class="mt-4 grid grid-cols-2 gap-2">
              <button @click="abrirEdicion(item, 'tcg')" class="bg-sky-500/10 hover:bg-sky-500 text-sky-400 hover:text-white py-2 rounded-xl flex items-center justify-center gap-2 text-xs font-black uppercase transition-all"><Edit class="w-4 h-4" /> Editar</button>
              <button @click="eliminarPublicacion('tcg_exchange', item, 'tcg')" class="bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white py-2 rounded-xl flex items-center justify-center gap-2 text-xs font-black uppercase transition-all"><Trash2 class="w-4 h-4" /> Borrar</button>
            </div>
          </div>
        </div>
      </div>
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div v-for="item in misArticulos" :key="item.id" class="bg-slate-800 rounded-2xl p-4 border border-slate-700 flex flex-col justify-between">
            <div class="flex gap-4 items-start">
              ...
              <div><span class="text-[9px] bg-purple-900/30 text-purple-400 px-2 py-1 rounded uppercase font-bold">{{ item.categoria }}</span><h4 class="font-bold text-white leading-tight mt-2">{{ item.item_nombre }}</h4><p class="text-sm text-green-400 font-black mt-1">${{ formatearPrecio(item.precio) }}</p></div>
            </div>
            <div class="mt-4 grid grid-cols-2 gap-2">
              <button @click="abrirEdicion(item, 'vitrina')" class="bg-purple-500/10 hover:bg-purple-500 text-purple-400 hover:text-white py-2 rounded-xl flex items-center justify-center gap-2 text-xs font-black uppercase transition-all"><Edit class="w-4 h-4" /> Editar</button>
              <button @click="eliminarPublicacion('colecciones', item, 'vitrina')" class="bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white py-2 rounded-xl flex items-center justify-center gap-2 text-xs font-black uppercase transition-all"><Trash2 class="w-4 h-4" /> Borrar</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-show="activeTab === 'lfg'" class="animate-in slide-in-from-left-4 duration-300">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div v-for="item in misGrupos" :key="item.id" class="bg-slate-800 p-5 rounded-2xl border border-slate-700 flex flex-col justify-between">
            ...
            <div class="mt-4 grid grid-cols-2 gap-2">
              <button @click="abrirEdicion(item, 'lfg')" class="bg-green-500/10 hover:bg-green-500 text-green-400 hover:text-white py-2 rounded-xl flex items-center justify-center gap-2 text-xs font-black uppercase transition-all"><Edit class="w-4 h-4" /> Editar</button>
              <button @click="eliminarPublicacion('lfg_posts', item, 'lfg')" class="bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white py-2 rounded-xl flex items-center justify-center gap-2 text-xs font-black uppercase transition-all"><Trash2 class="w-4 h-4" /> Borrar</button>
            </div>
          </div>
        </div>
      </div>
